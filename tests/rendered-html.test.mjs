...
test("server-renders a cited case file", async () => {
  ...
  assert.match(
    html,
    /return is at the seller(?:'|’|&#x27;|&#39;)s expense under Chapter 8, Section 2/i,
  );
  ...
  assert.match(
    html,
    /Start here if you want hardened replacements or a price reduction/i,
  );
  assert.match(html, /Ending the purchase and getting your money back/i);
  ...
  assert.equal((html.match(/Copy template/gi) ?? []).length, 2);
  ...
  // the claim letter offers the remedy at the seller's election, which is what closed the known case
  assert.match(html, /I claim, at your election/i);
  // replacement acceptance criterion stays measurable
  assert.match(html, /hardened through, not surface treated/i);
  // long-stop date, money fallback and interim wear cover are the settlement shape
  assert.match(
    html,
    /tools worn out by reasonable use before then are replaced or refunded/i,
  );
  assert.match(html, /the price reduction above falls due if that date passes/i);
  // the reply window is never dressed up as statutory
  assert.match(html, /That window is mine, not set by law/i);
  // goodwill substitutes are refused up front
  assert.match(html, /I do not accept a voucher, store credit or discount/i);
  // both letters are framed as a formal reklamation
  assert.match(html, /This is my formal reklamation under Swedish consumer law/i);
  ...
  // installed kits are not dismantled by the buyer, and a technician is demanded
  assert.match(html, /I will not dismantle it/i);
  assert.match(
    html,
    /send a technician to remove the INDX and restore my printer, at your cost/i,
  );
  // refund turns on proof of dispatch, not on the seller booking the return in
  assert.match(html, /Payment falls due on that evidence, not on booking-in/i);
  // no remedy conditional on a release, a gag or dropping a dispute
  assert.match(
    html,
    /conditional on signing a release, keeping it confidential, withdrawing a payment dispute/i,
  );
  ...
  for (const slug of [
    ...
    if (slug === "payment-surcharges" || slug === "nozzlegate") {
      ...
      if (slug === "payment-surcharges") {
        ...
        assert.equal(
          (remedyBlock.match(/Chapter 5, Section 5 of Konsumentköplag/g) ?? [])
            .length,
          1,
        );
        assert.equal(
